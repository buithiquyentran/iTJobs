...
def scrape_detail(link):
    ...
    try:
        # Banner
        IMG = soup.find('div', id="company-header-section").find('img')['src']
        print(IMG)
        # # LINH_VUC
...
def update_company_detail(ma_ntd, data):
    try:
        # Kiểm tra dữ liệu trước khi thực thi câu lệnh SQL
        if not data.get("IMG") or not ma_ntd:
            raise ValueError("Dữ liệu IMG hoặc MA_NTD không hợp lệ.")
        
        # Thực thi câu lệnh SQL
        cursor.execute("""
            UPDATE NHA_TUYEN_DUNG
            SET IMG = %s WHERE MA_NTD = %s
        """, (data["IMG"], ma_ntd))
        ...
        return  # Trả về ngay lập tức mà không thực hiện thêm gì

...
// import Box from "@mui/material/Box2";
const CompanyDetails = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const cpn = await axios.get(`http://localhost:5000/employers/${id}`);
                setCompany(cpn.data);

                const lv = await axios.get(`http://localhost:5000/employers/${id}/linh-vuc`);
                setLinhVuc(lv.data.LINH_VUC);

                const kn = await axios.get(`http://localhost:5000/employers/${id}/ki-nang`);
                setKiNang(kn.data.KI_NANG);

                const jobs = await axios.get(`http://localhost:5000/recruitments/company/${id}`);
                setJobs(jobs.data);
            } catch (error) {
                ...
                <Box item xs={12} md={4}>
                    ...
                    <Card sx={{ padding: 2, marginBottom: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, marginTop: 1 }}>
                            {kiNang?.length > 0 &&
                                kiNang.map((skill) => <Chip key={skill} label={skill} color="primary" />)}
                        </Box>
                    </Card>

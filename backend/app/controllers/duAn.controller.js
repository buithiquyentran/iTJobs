...
exports.getAll = async (req, res) => {
  try {
    const response = await DuAnService.getAll();
    res.json(response);
  } catch (error) {
...
exports.getOne = async (req, res) => {
  try {
    const response = await DuAnService.findById(req.params.id);
    res.json(response);
  } catch (error) {
    console.error("Error fetching", error.message);
    res.status(500).json({ message: "Error fetching data" });
...
exports.update = async (req, res, next) => {
  ...
  try {
    const document = await DuAnService.update(req.params.id, req.body);
    if (!document) {
...
exports.create = async (req, res, next) => {
  ...
  try {
    const document = await DuAnService.create(req.body);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }
    res.send(document);
...
exports.delete = async (req, res, next) => {
  try {
    const document = await DuAnService.delete(req.params.id);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }

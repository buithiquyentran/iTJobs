const { sequelize, KiNang } = require("../models");
const KiNangService = require("../services/kiNang.service");
exports.getAll = async (req, res) => {
  try {
    const response = await KiNangService.getAll();
    res.json(response);
  } catch (error) {
...
exports.getOne = async (req, res) => {
  try {
    const response = await KiNangService.findById(req.params.id);
    res.json(response);
  } catch (error) {
    console.error("Error fetching", error.message);
    res.status(500).json({ message: "Error fetching data" });
...
exports.update = async (req, res, next) => {
  ...
  try {
    const document = await KiNangService.update(req.params.id, req.body);
    if (!document) {
...
exports.create = async (req, res, next) => {
  ...
  try {
    const document = await KiNangService.create(req.body);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }
...
exports.delete = async (req, res, next) => {
  try {
    const document = await KiNangService.delete(req.params.id);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }

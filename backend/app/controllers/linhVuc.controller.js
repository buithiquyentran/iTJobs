const LinhVucService = require("../services/linhVuc.service");
exports.getAll = async (req, res) => {
  try {
    const response = await LinhVucService.getAll();
    res.json(response);
  } catch (error) {
...
exports.getOne = async (req, res) => {
  try {
    const response = await LinhVucService.findById(req.params.id);
    res.json(response);
  } catch (error) {
    console.error("Error fetching", error.message);
    res.status(500).json({ message: "Error fetching data" });
...
exports.update = async (req, res, next) => {
  ...
  try {
    const document = await LinhVucService.update(req.params.id, req.body);
    if (!document) {
...
exports.create = async (req, res, next) => {
  ...
  try {
    const document = await LinhVucService.create(req.body);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }
...
exports.delete = async (req, res, next) => {
  try {
    const document = await LinhVucService.delete(req.params.id);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }

...
exports.getOne = async (req, res) => {
  try {
    const response = await NguoiLaoDongService.findById(req.params.id);
    res.json(response);
  } catch (error) {
    console.error("Error fetching", error.message);
    res.status(500).json({ message: "Error fetching data" });
...
exports.update = async (req, res, next) => {
  ...
  try {
    const document = await NguoiLaoDongService.update(req.params.id, req.body);
    if (!document) {
...
exports.create = async (req, res, next) => {
  ...
  try {
    const document = await NguoiLaoDongService.create(req.body);
...
exports.delete = async (req, res, next) => {
  try {
    const document = await NguoiLaoDongService.delete(req.params.id);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }

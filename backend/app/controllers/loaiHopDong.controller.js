const LoaiHopDongService = require("../services/loaiHopDong.service");
exports.getAll = async (req, res) => {
  try {
    const response = await LoaiHopDongService.getAll();
    res.json(response);
  } catch (error) {
...
exports.getOne = async (req, res) => {
  try {
    const response = await LoaiHopDongService.findById(req.params.id);
    res.json(response);
  } catch (error) {
    console.error("Error fetching", error.message);
    res.status(500).json({ message: "Error fetching data" });
...
exports.update = async (req, res, next) => {
  ...
  try {
    const document = await LoaiHopDongService.update(req.params.id, req.body);
    if (!document) {
...
exports.create = async (req, res, next) => {
  ...
  try {
    const document = await LoaiHopDongService.create(req.body);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }
...
exports.delete = async (req, res, next) => {
  try {
    const document = await LoaiHopDongService.delete(req.params.id);
    if (!document) {
      return next(new ApiError(404, "Enity not found"));
    }
